...
namespace BT2
{
    public partial class Form1 : Form
    {
        ...
        private void New_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            SqlCommand cmd = new SqlCommand("insert into customerX values("+tbId.Text+",'"+tbName.Text+"')", conn);
            cmd.ExecuteNonQuery();
            conn.Close();
        ...
        private void Delete_Click(object sender, EventArgs e)
        {
            ...
            conn.ConnectionString = @"Data Source = localhost; Initial Catalog=SALES; User Id=sa; Password= sa";
            conn.Open();
            SqlCommand cmd = new SqlCommand("delete from customerX where id=" + tbId.Text , conn);
            cmd.ExecuteNonQuery();
            conn.Close();
        ...
        private void Edit_Click(object sender, EventArgs e)
        {
            ...
            conn.ConnectionString = @"Data Source = localhost; Initial Catalog=SALES; User Id=sa; Password= sa";
            conn.Open();
            SqlCommand cmd = new SqlCommand("update customerX set Name='" + tbName.Text + "' where id=" + tbId.Text , conn);
            cmd.ExecuteNonQuery();
            conn.Close();
            int idx = drvCustomer.CurrentCell.RowIndex;
